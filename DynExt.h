...
//template<class T>
//inline bool IsSet(T flags, T flag)
inline bool IsSet(DWORD flags, DWORD flag)
{
	if (flags & flag)
	{
		return true;
...
inline long& PackFloat(const float& f)
{
	return *((long*)&f);
}
inline float& UnpackFloat(const long& f)
{
	return *((float*)&f);
}
inline long PackString(const char* s)
{
...
double getAlterableValue(headerObject* object, int ValueNum);
...
__inline rCom* getrCom(headerObject* object)
{
	...
	if(!IsSet(OEFlags, OEFLAG_MOVEMENTS) && !IsSet(OEFlags, OEFLAG_ANIMATIONS))
		return 0;
...
{
	DWORD OEFlags = object->hoOEFlags;
	if(!IsSet(OEFlags, OEFLAG_MOVEMENTS))
		return 0;
...
{
	DWORD OEFlags = object->hoOEFlags;
	if(!IsSet(OEFlags, OEFLAG_ANIMATIONS))
		return 0;

	return (rAni*)((__int8*)object + sizeof(headerObject) + sizeof(rCom) +
		(IsSet(OEFlags, OEFLAG_MOVEMENTS) ? sizeof(rMvt) : 0));
}

__inline rSpr* getrSpr(headerObject* object)
{
	DWORD OEFlags = object->hoOEFlags;
	if(!IsSet(OEFlags, OEFLAG_SPRITES))
		return 0;

	return (rSpr*)((__int8*)object + sizeof(headerObject) +
		((IsSet(OEFlags, OEFLAG_MOVEMENTS) ||
		 IsSet(OEFlags, OEFLAG_ANIMATIONS)) ? sizeof(rCom) : 0) +
		(IsSet(OEFlags, OEFLAG_MOVEMENTS) ? sizeof(rMvt) : 0) +
		(IsSet(OEFlags, OEFLAG_ANIMATIONS) ? sizeof(rAni) : 0));
}

__inline rVal* getrVal(headerObject* object)
{
	DWORD OEFlags = object->hoOEFlags;
	if(!IsSet(OEFlags, OEFLAG_VALUES))
		return 0;
	...
	return (rVal*)((__int8*)object + sizeof(headerObject) +
		...
		(IsSet(OEFlags, OEFLAG_MOVEMENTS) ? sizeof(rMvt) : 0) +
		(IsSet(OEFlags, OEFLAG_ANIMATIONS) ? sizeof(rAni) : 0) +
		(IsSet(OEFlags, OEFLAG_SPRITES) ? sizeof(rSpr) : 0));
}
