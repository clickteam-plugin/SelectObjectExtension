...
extern "C" int WINAPI DLLExport Initialize(mv _far *mV, int quiet)
{
	...
	else
	{
		if (!quiet)
		{
			MessageBox(0, "This version of the Select Object is not compatible with your version of MMF2. Please look for an update.\n\nThankyou, and have a nice day.", "Select Object Extension", MB_OK);
		}
		return 1;
	}
}
...
extern "C" 
{
	DWORD WINAPI DLLExport GetInfos(int info)
	{
		...
		switch (info)
		{
			...
			case KGI_BUILD:
				return MINBUILD;					// Works with build 244 or above
			default:
				return 0;
		}
	}
